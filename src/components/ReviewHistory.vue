...
const periodId = ref("reviewHistory1");
const grouping = ref<'Days'|'Weeks'|'Months'>("Days");
...
function checkResize() {
  ...
  let hideDates = false;
  const reviewHistoryCard = cardsStore.cards.find((c) => c.item.i === "ReviewHistory");
...
watch(
  ...
  async ([lang, deckId, period, group], _prev, onCleanup) => {
    if (!lang) return;
    const fetchPromise = reviewHistoryStore.fetchReviewHistoryIfNeeded(lang, deckId, period, group);
    let cancelled = false;
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: function (context: any) {
            ...
            const allData = context.chart.data.datasets.reduce((acc: number, ds: any) => acc + (Array.isArray(ds.data) ? ds.data[context.dataIndex] ?? 0 : 0), 0);
...
<template>
  ...
  >
    ...
    >
      <template
        v-if="
          reviewHistory && reviewHistory.labels && reviewHistory.counts && !isLoading && !error
        "
      >
        <Bar
          :data="chartData"
